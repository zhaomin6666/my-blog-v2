...
- Blog Admin ?? `lib/admin/blog-admin-service.ts`?
- SQL ??? `lib/admin/blog-admin-repository.ts`?
- ??? Client Component ????? PostgreSQL?
- SQL ????????

??????

- Blog Admin ?? PostgreSQL `blog_posts`?
- ?? Blog ????? `BLOG_CONTENT_SOURCE=database` ? `CONTENT_SOURCE=database` ????????
- ?? file ????????
- ?????????????????? `content/blog`?

???????title?slug?summary?status?lang?date?tags?series?series slug?series order?cover?SEO title?SEO description?Markdown content?

?????????? `draft`?????? `published_at` ??????????????? `draft` ??? `published_at`??? / ?? / ??? revalidate Blog?Search?Tags?Series?Sitemap?RSS ??????????

Slug ????? active database posts ????????????????? hyphen?

? MVP ??? Projects Admin?Profile / Homepage Admin?Content import / export?????????????AI ????????Console / CLI ?????????? Docker / Nginx ???????
