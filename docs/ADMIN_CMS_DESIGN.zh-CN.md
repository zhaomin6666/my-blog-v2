...
- middleware 保护 `/admin/*`，并放行 `/admin/login`。
- 通过 `ADMIN_USERNAME`、`ADMIN_PASSWORD_HASH` 和 `ADMIN_SESSION_SECRET` 使用环境变量管理员凭据。
- 基础登录限流。
- Admin 路由使用 `noindex`。

第一版仍然刻意避免公开注册、多管理员管理、`admin_users`、审计日志和角色分离。

### Phase 11.5：Blog Admin MVP - 已实现
...
- Blog Admin 使用 `lib/admin/blog-admin-service.ts`。
- SQL 集中在 `lib/admin/blog-admin-repository.ts`。
- 页面和 Client Component 不直接连接 PostgreSQL。
- 所有 SQL 都使用参数化查询。
...
- SEO title
- SEO description
- Markdown content，使用普通 `textarea`

发布规则：

- 新文章默认是 `draft`。
- Draft 不会出现在公开 database-mode Blog 页面。
- 发布会设置 `status = 'published'`，并仅在 `published_at` 为空时填充发布时间。
- 下架会设置 `status = 'draft'`，保留 `published_at` 作为历史发布时间元数据。
- 保存 / 发布 / 下架会 revalidate Blog、Search、Tags、Series、Sitemap、RSS，以及可确定的文章详情路径。

Slug 规则：

- Slug 必填。
- Slug 必须在 active database posts 中全局唯一。
- 编辑时可以保留当前文章 slug。
- Slug 只接受小写字母、数字和连字符。
- 空格、中文、斜杠、查询字符和其它 URL 特殊字符会被拒绝。

此 MVP 不包含：Projects Admin、Profile / Homepage Admin、内容导入导出、图片上传、富文本编辑器、AI 写作、评论、Console / CLI 变更、窗口系统变更、Docker / Nginx 部署配置变更。

### Phase 11.6：Homepage / Profile Admin - 已实现

- 新增 `/admin/hero`、`/admin/profile`、`/admin/contact` 和 `/admin/stack`。
- Profile / Contact / Stack 公开读取继续通过 `ProfileService`。
- file mode 继续读取 `content/profile`，不受数据库后台内容影响。
- 未修改 Agent Demo 回答范围、Console / CLI、窗口系统或部署配置。

### Phase 11.7：Projects Admin - 已实现

- 新增 `/admin/projects`、`/admin/projects/new` 和 `/admin/projects/[id]`。
- Projects Admin 写入 PostgreSQL `projects`。
- 公开 Projects 只有在 `PROJECT_CONTENT_SOURCE=database` 或 `CONTENT_SOURCE=database` 时读取数据库项目。
- file mode 继续读取 `content/projects`。

### Phase 11.8：Admin Markdown Import / Export - 已实现

已实现受登录保护的 Blog / Projects Markdown 导入导出能力。Phase 11.8-fix 后，导入导出不再通过独立 `/admin/content` 页面访问：Blog 工具位于 `/admin/blog`，Project 工具位于 `/admin/projects`，底层 import/export service 行为保持不变。
...
- frontmatter 校验、slug 检查、逐文件报告和上传限制。
- Blog Posts / Projects 单篇 Markdown 导出。
- Blog Posts / Projects active rows 批量 zip 导出。
