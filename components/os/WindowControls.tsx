...
export function WindowControls({
  ...
  return (
    <div className="flex gap-2 w-20">
      <button
        onClick={(e) => { e.stopPropagation(); onClose(); }}
        className={`${tokens.controlSize} rounded-full ${tokens.controlClose} flex items-center justify-center`}
        aria-label={t('window.aria.close', lang)}
      >
      ...
      <button
        onClick={(e) => { e.stopPropagation(); onMinimize(); }}
        className={`${tokens.controlSize} rounded-full ${tokens.controlMinimize} flex items-center justify-center`}
        aria-label={t('window.aria.minimize', lang)}
      >
      ...
      <button
        onClick={(e) => { e.stopPropagation(); onMaximize(); }}
        className={`${tokens.controlSize} rounded-full ${tokens.controlMaximize} flex items-center justify-center`}
        aria-label={t('window.aria.maximize', lang)}
      >
        {isMaximized ? (
          <Minimize2 size={8} className="opacity-60 hover:opacity-100" />
