...
export async function GET(request: Request): Promise<Response> {
  ...
  const { buffer } = await contentTransferService.exportMarkdownZip('blog', readScope(request.url));

  return new Response(buffer, {
    headers: {
      'Content-Type': 'application/zip',
      'Content-Disposition': `attachment; filename="${zipDownloadFilename('blog')}"`,
    },
  });
}
