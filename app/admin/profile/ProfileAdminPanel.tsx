...
import { StructuredProfileForm } from './StructuredProfileForm';

interface ProfileAdminPanelProps {
  zhPage: AdminProfilePage;
  enPage: AdminProfilePage;
}

type ProfileLanguage = 'zh' | 'en';

const languageOptions: Array<{ value: ProfileLanguage; label: string }> = [
  { value: 'zh', label: 'zh' },
  { value: 'en', label: 'en' },
];

export function ProfileAdminPanel({ zhPage, enPage }: ProfileAdminPanelProps) {
  const [activeLanguage, setActiveLanguage] = useState<ProfileLanguage>('zh');
  const activePage = activeLanguage === 'zh' ? zhPage : enPage;
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <StructuredProfileForm key={activeLanguage} profilePage={activePage} />
      </div>
    </div>
  );
}
